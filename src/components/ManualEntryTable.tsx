...
const ManualEntryTable = forwardRef(({ offices, employees, onStaffingUpdate, onAttendanceChange }: ManualEntryTableProps, ref) => {
    ...
    getSummaryData: () => {
      ...
      offices.forEach(office => {
        const absentEmployeeNames = employees
          .filter(emp => emp.officeId === office.id && emp.status === 'Ausente' && emp.absenceReason && !PROLONGED_ABSENCE_REASONS.includes(emp.absenceReason))
          .map(emp => emp.name)
          .join(' / ');

        const prolongedAbsenceNames = employees
          .filter(emp => emp.officeId === office.id && emp.absenceReason && PROLONGED_ABSENCE_REASONS.includes(emp.absenceReason))
          .map(emp => emp.name)
          .join(' / ');
  ...
  const getEmployeeNamesByStatus = (officeId: string, status: AttendanceStatus): React.ReactElement | "-" => {
    const filteredEmployees = employees
      ...
      .filter(emp => {
        if (status === 'Ausente') {
          return emp.absenceReason && !PROLONGED_ABSENCE_REASONS.includes(emp.absenceReason);
        }
        return true;
      });
  ...
  const getEmployeeNamesForProlongedAbsence = (officeId: string): React.ReactElement | "-" => {
    ...
    const names = officeEmployees
      .filter(emp => emp.absenceReason && PROLONGED_ABSENCE_REASONS.includes(emp.absenceReason))
      .map(emp => emp.name);
                                  ...
                                  {activeEmployees.map(emp => (
                                    <div key={emp.id} className="flex items-center justify-between">
                                      <Label htmlFor={`attendance-${office.id}-${emp.id}`} className="flex-1 cursor-pointer flex items-center gap-2">
                                        ...
                                        {emp.employmentType === 'Part-Time' && <Clock className="h-3.5 w-3.5 text-blue-500 shrink-0" />}
                                      </Label>
                                      ...
                                      {prolongedAbsenceEmployees.map(emp => (
                                        <div key={emp.id} className="flex items-center justify-between">
                                          <Label className="flex-1 flex items-center gap-2 text-muted-foreground italic">
                                            ...
                                            <span className="truncate">{emp.name}</span>
                                            {emp.employmentType === 'Part-Time' && <Clock className="ml-2 h-3.5 w-3.5 text-blue-500 shrink-0" />}
                                          </Label>
